...
client.on('messageCreate', async (message) => {
    ...
    try {
        ...
        for (const url of urls) {
            const row = db.prepare('SELECT 1 FROM spam_assets WHERE type = "url" AND value = ?').get(url);
            if (row) {
                isSpam = true;
                break;
            }
        ...
        if (!isSpam && message.attachments.size > 0) {
            for (const attachment of message.attachments.values()) {
                ...
                const row = db.prepare('SELECT 1 FROM spam_assets WHERE type = "hash" AND value = ?').get(currentHash);
                if (row) {
                    isSpam = true;
                    break;
                }
